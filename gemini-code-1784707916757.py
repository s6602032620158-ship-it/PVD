...
def render_pvd_svg(pattern, S_m, de_m):
    p_text = (
        "Square Grid (d_e = 1.13S)"
        if pattern == "square"
        else "Triangular Grid (d_e = 1.05S)"
    )
    ...
    <svg viewBox="0 0 400 240" width="100%" height="220" xmlns="http://www.w3.org/2000/svg">
        ...
        <text x="200" y="32" font-size="12" fill="#2563EB" font-weight="600" text-anchor="middle">Zone of Influence (de = {de_m:.2f} m)</text>
        
        <!-- Neighbor PVDs -->
        <rect x="94" y="114" width="12" height="12" rx="2" fill="url(#pvdGrad)" opacity="0.5"/>
        <rect x="294" y="114" width="12" height="12" rx="2" fill="url(#pvdGrad)" opacity="0.5"/>
        <rect x="194" y="34" width="12" height="12" rx="2" fill="url(#pvdGrad)" opacity="0.5"/>
        <rect x="194" y="194" width="12" height="12" rx="2" fill="url(#pvdGrad)" opacity="0.5"/>
        
        <!-- Center PVD Point -->
        <rect x="194" y="114" width="12" height="12" rx="2" fill="url(#pvdGrad)"/>
        <text x="200" y="145" font-size="11" fill="#1E293B" font-weight="600" text-anchor="middle">Center PVD</text>
        
        <!-- Dimension Line for S -->
        <line x1="200" y1="120" x2="294" y2="120" stroke="#0F172A" stroke-width="1.5" marker-end="url(#arrow)"/>
        <text x="247" y="112" font-size="11" fill="#0F172A" font-weight="700" text-anchor="middle">S = {S_m:.2f} m</text>
        
        <!-- Grid Pattern Type Text -->
        <text x="25" y="32" font-size="12" fill="#78350F" font-weight="700">{p_text}</text>
    </svg>
    """
    return svg
    ...
    if err_msg:
        ...
        with img_col1:
            st.markdown(
                ...
                <div class="diagram-box">
                    <div style="font-weight:700; color:#0F172A; margin-bottom:0.2rem;">📱 Dynamic Cross-Section Diagram</div>
                    <div style="font-size:0.825rem; color:#64748B; margin-bottom:0.8rem;">ผังการจัดวาง PVD และ รัศมีอิทธิพล (Zone of Influence) ที่คำนวณตามจริง</div>
                </div>
            """,
                unsafe_allow_html=True,
            )
            # Render SVG ไดนามิกอย่างถูกต้องผ่าน st.html
        ...
        with img_col2:
            st.markdown(
                """
                <div class="diagram-box">
                    <div style="font-weight:700; color:#0F172A; margin-bottom:0.2rem;">🏞️ Soil & PVD Profile Illustration</div>
                    <div style="font-size:0.825rem; color:#64748B; margin-bottom:0.8rem;">ภาพจำลองโครงสร้างชั้นดินเหนียวอ่อนและการไหลคายน้ำของน้ำใต้ดิน</div>
                </div>
            """,
                unsafe_allow_html=True,
            )
        ...
        with st.expander(
            "📐 ตัวแปรคำนวณระหว่างทางเพิ่มเติม (Intermediate Results)"
        ):
        ...
        # 5.5 Advanced Interactive Analytics Charts
